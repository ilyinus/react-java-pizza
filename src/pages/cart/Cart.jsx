...
import { EmptyCart, CartItem } from "../index";
import { clearCart, addToCart, removeFromCart } from '../../redux/actions/cart'
...
export default function Cart() {
    ...
    const increaseCount = React.useCallback(item => {
    ...
    const decreaseCount = React.useCallback(item => {
    ...
    const removeItems = React.useCallback(item => {
        dispatch(removeFromCart(item))
        // eslint-disable-next-line
    }, [])

    const renderCartItems = () => {
        return Object.values(store.items).map((item, index) => {
            return (
                <CartItem
                    key={index}
                    item={item}
                    increaseCountHandler={increaseCount}
                    decreaseCountHandler={decreaseCount}
                    removeItemsHanddler={removeItems}
                />
            )
        })
    }
