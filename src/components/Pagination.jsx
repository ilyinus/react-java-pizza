...
const Pagination = ({ pag, handler }) => {
    ...
    return (
        <div className="pagination">
            <ul>
                <li className="arrow" onClick={() => handler('<')}>&lt;</li>
                {start > 1 && <li onClick={() => handler(1)}>1</li>}
                {start - 1 > 1 && <li className="gap">...</li>}
                {
                    Array(block).fill(0).map((_, index) => {
                        return (
                            <li key={index}
                ...
                {pag.totalPages > end && <li onClick={() => handler(pag.totalPages)}>{pag.totalPages}</li>}
                <li className="arrow" onClick={() => handler('>')} >&gt;</li>
            </ul>
        </div>
    )
